<?php 
	...
	while ($fila = mysql_fetch_row($resultado)){ 
       echo '<tr><td>'.$fila[1].'</td>
				 ...
				 <td>'.$fila[4].'</td>
				 <td>'.$fila[5].'</td>
	   <td><a class="eliminar_nuevo_detalle" href="javascript:void(0);" data-id="'.$fila[0].'" onclick="eliminar_nuevo_detalle_factura('.$fila[0].')">Borrar</a></td></tr>'; 
	}
	 
	$resultado = mysql_query("SELECT SUM(unidades) as suma_unidades, sum(monto_concepto) as suma_montos FROM detalle_factura_temp", $db); 
	...
	while ($fila = mysql_fetch_row($resultado)){ 
       echo '<tr><td style="color:#000; border:1px solid #c4c4c4; background:#6DA8C6;">Totales</td>
				 ...
				 <td style="color:#868686; border:1px solid #c4c4c4;">'.$fila[1].'</td></tr>';
	}
